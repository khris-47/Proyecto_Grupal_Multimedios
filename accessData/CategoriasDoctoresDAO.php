...
class CategoriasDoctoresDAO {
    ...
    public function obtenerTodos() {
        $stmt = $this->pdo->query("SELECT * FROM B86781_categorias_doctores;");
        $resultado = [];
    ...
    public function insertar(CategoriaDoctor $objeto) {
        $sql = "INSERT INTO B86781_categorias_doctores (doctor_id, categoria_id) VALUES (?, ?);";
    ...
    public function actualizar(CategoriaDoctor $objeto) {
        $sql = "UPDATE B86781_categorias_doctores SET doctor_id = ?, categoria_id = ? WHERE id = ?;";
        $stmt = $this->pdo->prepare($sql);
    ...
    public function eliminar($id) {
        $sql = "DELETE FROM B86781_categorias_doctores WHERE id = ?;";
        $stmt = $this->pdo->prepare($sql);
